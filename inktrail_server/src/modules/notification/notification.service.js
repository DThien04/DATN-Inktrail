...
// Sửa chuỗi tiếng Việt bị lỗi mã hóa trước khi lưu/phát thông báo.
const normalizeDisplayText = (value) => {
  const normalized = normalizeOptionalString(value);
  if (!normalized) return normalized;

  // Dò dấu hiệu chuỗi lỗi kiểu UTF-8 bị đọc sai (mojibake).
  // Dùng unicode escape để tránh lỗi mã hóa ngay trong source.
  const looksBroken = /[\u00C3\u00C2]|[\u00E2]\u20AC|[\u00E1][\u00BA\u00BB]|�/u.test(
    normalized,
  );
...
const createNotification = async ({
  ...
  const notification = await prisma.notification.create({
    data: {
      ...
      body: normalizeDisplayText(body),
