...
const buildChapterModerationInput = (chapter) =>
  [normalizeText(chapter?.title), normalizeText(chapter?.content)]
    .filter(Boolean)
    .join("\n\n");

const buildChapterModerationHash = (chapter) =>
  buildContentHash(buildChapterModerationInput(chapter));
...
const processChapterModeration = async ({
  ...
  try {
    ...
    const moderationInput = buildChapterModerationInput(chapter);
    ...
    if (!blocked) {
      await prisma.chapter.updateMany({
        where: { id: chapter.id },
        data: {
          lastApprovedContentHash: buildChapterModerationHash(chapter),
    ...
    const updated = await prisma.chapter.updateMany({
      ...
      data: {
        lastRejectedContentHash: buildChapterModerationHash(chapter),
...
const publishChapter = async ({ chapterId, requester }) => {
  ...
  if (
    ...
    chapter.lastRejectedContentHash &&
